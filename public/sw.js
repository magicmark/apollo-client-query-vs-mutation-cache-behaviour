...
spurious results.`);
  ...
  var __Schema = new GraphQLObjectType({
    ...
    fields: () => ({
      ...
      directives: {
        ...
        resolve: (schema2, { includeDeprecated }) => includeDeprecated ? schema2.getDirectives() : schema2.getDirectives().filter((directive) => directive.deprecationReason == null)
      }
    })
  });
  ...
  function execute(args) {
    ...
    const { schema: schema2, document, variableValues, rootValue: rootValue2 } = args;
    ...
    try {
      ...
      const result = executeOperation(exeContext, operation, rootValue2);
      if (isPromise(result)) {
        return result.then(
  ...
  function executeOperation(exeContext, operation, rootValue2) {
    ...
    switch (operation.operation) {
      case OperationTypeNode.QUERY:
        return executeFields(exeContext, rootType, rootValue2, path, rootFields);
      case OperationTypeNode.MUTATION:
