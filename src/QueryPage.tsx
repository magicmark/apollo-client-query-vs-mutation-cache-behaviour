...
export default function QueryPage() {
  ...
  const [rollDice, { data, loading }] = useLazyQuery<{ diceRoll: { value: number } }>(DICE_ROLL_QUERY);

  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Query)</h1>
      ...
      {data?.diceRoll.value != null && (
        <div style={styles.results}>
          <p style={{ fontSize: "1.75rem", margin: "0.25rem 0" }}>
            You rolled: <strong>{data.diceRoll.value}</strong>
          </p>
        </div>
      )}
