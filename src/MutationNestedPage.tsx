...
export default function MutationNestedPage() {
  const client = useApolloClient();
  const [result, setResult] = useState<number | null>(null);
  const [rollDice, { loading }] = useMutation<{ updateUser: { diceRoll: { value: number } } }>(UPDATE_USER_MUTATION, {
    onCompleted: (data) => {
      setResult(data.updateUser.diceRoll.value);
    },
  });

  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Nested Mutation)</h1>
