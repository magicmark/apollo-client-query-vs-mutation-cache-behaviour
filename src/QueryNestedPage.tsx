...
export default function QueryNestedPage() {
  const client = useApolloClient();
  const [rollDice, { data, loading }] = useLazyQuery<{ user: { diceRoll: { value: number } } }>(USER_DICE_ROLL_QUERY);

  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Nested Query)</h1>
      ...
      {data?.user.diceRoll.value != null && (
        <div style={styles.results}>
          <p style={{ fontSize: "1.75rem", margin: "0.25rem 0" }}>
            You rolled: <strong>{data.user.diceRoll.value}</strong>
          </p>
        </div>
      )}
