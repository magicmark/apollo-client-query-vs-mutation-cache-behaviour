...
const OPERATION_TEXT = `query DiceRollQuery {
  diceRoll {
    value
  }
}`;

const DICE_ROLL_QUERY = gql`
  query DiceRollQuery {
    diceRoll {
      value
    }
...
function RollRow({ rollId }: { rollId: number }) {
  const { data, loading } = useQuery<{ diceRoll: { value: number } }>(DICE_ROLL_QUERY, {
...
export default function QueryPersistedPage() {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Query - Persisted)</h1>
