...
const OPERATION_TEXT = `mutation UpdateUserMutation {
  updateUser(id: 123, newPassword: "hunter2") {
    id
    diceRoll {
      value
    }
...
function MutationRollRow({ rollId }: { rollId: number }) {
  const [execute, { data, loading }] = useMutation<{ updateUser: { id: string; diceRoll: { value: number } } }>(UPDATE_USER_MUTATION);
  ...
  return (
    <p style={{ fontSize: "1.75rem", margin: "0.25rem 0" }}>
      {loading ? "Rolling..." : data ? <>You rolled: <strong>{data.updateUser.diceRoll.value}</strong></> : null}
    </p>
  );
}
...
export default function MutationNestedPersistedPage() {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Nested Mutation - Persisted)</h1>
