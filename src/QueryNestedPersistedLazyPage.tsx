...
const OPERATION_TEXT = `query UserDiceRollQuery {
  user(id: 123) {
    id
    diceRoll {
      value
...
const USER_DICE_ROLL_QUERY = gql`
  query UserDiceRollQuery {
    user(id: 123) {
      id
      diceRoll {
        value
...
function LazyQueryRollRow({ rollId }: { rollId: number }) {
  const [execute, { data, loading }] = useLazyQuery<{ user: { diceRoll: { value: number } } }>(USER_DICE_ROLL_QUERY, {
  ...
  return (
    <p style={{ fontSize: "1.75rem", margin: "0.25rem 0" }}>
      {loading ? "Rolling..." : data ? <>You rolled: <strong>{data.user.diceRoll.value}</strong></> : null}
    </p>
  );
}
...
export default function QueryNestedPersistedLazyPage() {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Dice Roller (Nested Query - Persisted - Lazy)</h1>
      ...
      {rollIds.length > 0 && (
        <div style={styles.results}>
          {rollIds.map((id) => (
            <LazyQueryRollRow key={id} rollId={id} />
          ))}
        </div>
      )}
